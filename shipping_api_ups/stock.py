...
class stock_picking(models.Model):
    ...
    def on_change_sale_id(self, sale_id=False, state=False):
        ...
        if sale_id:
            ...
            service_type_obj = self.env['ups.shipping.service.type']
            ups_shipping_service_ids = service_type_obj.search([('description', '=', sale_obj.ship_method)])
            if ups_shipping_service_ids:
                vals['ups_service'] = ups_shipping_service_ids[0]
                shipping_obj = self.env['ups.account.shipping']
                ups_shipping_ids = shipping_obj.search([('ups_shipping_service_ids', 'in', ups_shipping_service_ids[0])])
                if ups_shipping_ids:
                    vals['shipper'] = ups_shipping_ids[0]
                    log_company_obj = self.env['logistic.company']
                    logistic_company_ids = log_company_obj.search([('ups_shipping_account_ids', 'in', ups_shipping_ids[0])])
                    if logistic_company_ids:
                        vals['logis_company'] = logistic_company_ids[0]
        return {'value': vals}
    
    @api.multi
    def onchange_bill_shipping(self, bill_shipping, ups_use_cc, ups_cc_address_id, ups_bill_receiver_address_id, partner_id, shipper):
        vals = {}
        if bill_shipping == 'shipper':
            if not ups_cc_address_id and shipper:
                ship_address = self.env['ups.account.shipping'].read(['address'])
                if ship_address:
...
#    trade_mark =                    fields.Char(related='shipper', size=1024, string='Trademark')
#    ship_company_code =             fields.Selection(_get_company_code, string='Ship Company', size=64)
#    ups_pickup_type =               fields.Selection([
#                                        ('01', 'Daily Pickup'),
#                                        ('03', 'Customer Counter'),
...
#         return super(stock_picking_out, self).action_process(cr, uid, ids, context=context)
    ...
    def action_done(self):
        ...
        for picking in self.browse(self._ids):
            vals = {}
            service_type_obj = self.env['ups.shipping.service.type']
            ship_method = picking.sale_id and picking.sale_id.ship_method
            if ship_method:
                service_type_ids = service_type_obj.search([('description', 'like', ship_method)])
                if service_type_ids:
                  vals['ups_service'] = service_type_ids[0]
                  service_type = service_type_obj.browse(service_type_ids)[0]
...
#        return {'value': vals}
    ...
    def process_void(self):
        ...
        if do:
            ...
            if void_web:
                ...
                raise except_orm(_('Unable to find Shipping URL!'), _("Please configure the shipping company with websites."))
                    ...
                    for item in elm['Response']:
                        if 'ResponseStatusCode' in item:
                            if item['ResponseStatusCode'] == '1':
                                ...
                                self.env['stock.packages'].write(clear_vals)
                                picking_obj.write({'ship_state': 'draft', 'ship_message': 'Shipment has been cancelled.'})
                                break
                        ...
                        if 'Error' in item:
                            for i in item['Error']:
                                if 'ErrorDescription' in i:
                                    ...
                                    picking_obj.write({'ship_message': status_description + ': ' + error_description })
                                    break
            return status
        return False
    ...
    @api.multi
    def create_ship_accept_request_new(self, do):
        if not do.shipper:
            return ''
    ...
    @api.multi
    def process_ship_accept(self, do, packages):
        shipment_accept_request_xml = self.create_ship_accept_request_new(do)
        if do.logis_company.test_mode:
                                ...
                                if tracks.get('LabelImage'):
                                    for label in tracks['LabelImage']:
                                        ...
                                        if label.get('GraphicImage'):
                                            ...
                                            package_data = package_obj.read(packages_ids[packages], ['weight', 'description'])
#                                             i += 1
                                            ship_move_obj = self.env['shipping.move']
                                            if label_code == 'GIF':
                                                ship_move_obj.create({
                                                    'pick_id': do.id,
                        ...
                        if package.get('ControlLogReceipt'):
                            for items in package['ControlLogReceipt']:
                                if items.get('GraphicImage'):
                                    ...
                                    label_from_file.close()
                                    package_obj.write({'control_log_receipt': control_log_image, })
            do.write({'ship_state': 'ready_pick', 'ship_charge': ship_charge, 'internal_note': tracking_number_notes})
        return status, label_code

    @api.multi
    def add_product(self, package_obj):
        prods = []
        tot_weight = 0
    ...
    @api.multi
    def create_comm_inv(self, package_obj):
        invoice_id = False
        ...
        return comm_inv
    @api.multi
    def create_cer_orig(self, package_obj):
        cer_orig = {
            'FormType': "03",
            'Product': [],  # Placed out of this dictionary for common use
    ...
    @api.multi
    def get_value(self, object, message=None):
    ...
    @api.multi
    def create_ship_confirm_request_new(self, do, lines):
        if not do.shipper:
            return ''
...
#         for lines in do.packages_ids:
    ...
    def process_ship(self):
        ...
        if not (deliv_order.logis_company or deliv_order.shipper or deliv_order.ups_service):
            raise except_orm(_("Warning"), _("Please select Logistics Company, Shipper and Shipping Service"))
        if not deliv_order.packages_ids:
            raise except_orm(_("Warning"), _("Please add shipping packages before doing Process Shipping."))
        if deliv_order.sale_id and deliv_order.sale_id.order_policy == 'credit_card' and not deliv_order.sale_id.invoiced:
            if not deliv_order.sale_id.cc_pre_auth:
                raise except_orm(_("Warning"), _("The sales order is not paid"))
            else:
                do_transaction = False
                ...
                if rel_voucher_id and rel_voucher_id.state != 'posted' and deliv_order.sale_id.cc_pre_auth:
                    ...
                    self.env['account.voucher'].write(vals_vouch)
                    do_transaction = self.env['account.voucher'].authorize([rel_voucher_id.id])
            if  not do_transaction:
                self.write({'ship_state': 'hold', 'ship_message': 'Unable to process creditcard payment.'})
                self._cr.commit()
                raise except_orm(_('Final credit card charge cannot be completed!'), _("Please hold shipment and contact customer service.."))
        warning_error = False
        status = 0
        if deliv_order:
            packages = 0
            for lines in deliv_order.packages_ids:
                ship_confirm_request_xml = self.create_ship_confirm_request_new(deliv_order, lines)
                ship_confirm_web = ''
                ship_confirm_port = ''
...
#                                                context.update({'error_message':err.get('ErrorDescription')})
                    ...
                    package_obj.write({'ship_message': status_description})
                    ...
                    if not status:
                        ...
                        for elm in response_dic['ShipmentConfirmResponse']:
                            if elm.get('ShipmentDigest'):
                                ...
                                break
                        deliv_order.write({'shipment_digest': shipment_digest, 'shipment_identific_no': shipment_identification_number, })
                        do = self.browse(deliv_order.id)
                        status = self.process_ship_accept(do, packages)[0]
                        label_code = self.process_ship_accept(do, packages)[1]
                packages += 1
                           
            if status:
                try:
                    self.send_conf_mail(do.id)
                except Exception, e:
                    pass
                if label_code == 'GIF':
                    return {
    ...
    def _get_journal_id(self):
        ...
        vals = []
        for pick in self.browse(self._ids):
            ...
            for jr_type in journal_obj.browse(value):
    ...
    def do_partial(self, partial_datas):
        ...
        result_partial = super(stock_picking, self).do_partial(partial_datas)
        if res and res[0]:
            journal_id = res[0][0]
            result = result_partial
            for picking_obj in self.browse(self._ids):
                sale = picking_obj.sale_id
                if sale and sale.order_policy == 'picking':
                    pick_id = result_partial[picking_obj.id]['delivered_picking']
                    result = self.action_invoice_create([pick_id], journal_id, type=None)
                    ...
                    if result:
                        ...
                        inv_obj.button_reset_taxes(result.values())
...
class stock_move(models.Model):
    ...
    def create(self, values):
        ...
        package_obj = self.env['stock.packages']
        pack_id = None
        package_ids = package_obj.search([('pick_id', "=", values.get('picking_id'))])
        if values.get('picking_id'):
            rec = self.env['stock.picking'].browse(values.get('picking_id'))
            if not context.get('copy'):
                if not package_ids:
                    pack_id = package_obj.create({'package_type': rec.sale_id.ups_packaging_type.id, 'pick_id': vals.get('picking_id')})
        res = super(stock_move, self).create(values)
        if not context.get('copy'):
